...
- 新增 `src/mini_claude/project_context.py`：规范化 workspace 根目录，生成稳定 `workspace_id`，并固定 `root`、配置根、`runtime_data_dir`、规则/技能/agents/MCP/memory 来源与工具 cwd。
...
- **兼容**：身份算法以 `realpath` 为唯一规范化来源，与历史算法 `sha256(str(Path.cwd()))[:16]` 取值一致（本机实测 `514fed37912aa299`），既有 `~/.mini-claude/projects/<hash>/memory/**` 不搬移、不复制、不失联（D3）。
- 旧会话保持立场为「不猜测归属、不写绑定、不改写数据」；workspace 绑定字段与版本**留给 C03**（D5）。
- 不移动历史数据目录、不改变 `MINI_CLAUDE_RUNTIME_DIR` 语义、不改变 CLI 默认「当前目录即 workspace」的对外行为。
...
- 新增：`src/mini_claude/project_context.py`、`src/mini_claude/tests/test_project_context.py`、`src/mini_claude/tests/conftest.py`（测试隔离强制点，闭合 GAP-I02-01）。
