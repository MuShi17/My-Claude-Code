...
- 新增统一的 Application API，提供 `session.create/list`、`run.start/status/cancel`、`interaction.respond` 与 `shutdown` 入口；命令携带稳定身份和参数摘要，重复命令按幂等规则返回已有结果。
...
- 增加版本化的 session/run/command/pending-interaction 控制记录，支持旧 session 读取与 schema 迁移；崩溃恢复把未完成执行标为中断或 uncertain，禁止自动重放可能产生副作用的工具。
