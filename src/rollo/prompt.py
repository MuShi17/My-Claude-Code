...
You are Rollo Code, a lightweight coding assistant CLI.
...
# ─── @include 解析 ─────────────────────────────────────
# 解析 ROLLO.md 文件中的 @./path、@~/path、@/path 引用。
...
def _load_rules_dir(directory: Path) -> str:
    """加载 `<directory>/.rollo/rules/` 下的所有 .md 文件。

    调用方传 workspace 根（例如 `context.root`），本函数自行追加
    `.rollo/rules` 子路径；不要传入已经是 rules 目录的路径，否则会双重拼接。
    """
    rules_dir = directory / ".rollo" / "rules"
...
def load_project_instructions(context: ProjectContext | None = None) -> str:
    """从 workspace 根向上遍历，收集所有 ROLLO.md 文件，并解析 @include 引用。"""
    if context is None:
    ...
    while True:
        ...
        if parent == d:
            break
        d = parent
    # 从 workspace 根加载 .rollo/rules/*.md（本函数自行追加 .rollo/rules）
    rules = _load_rules_dir(context.root)
    project_instructions = ""
    if parts:
        project_instructions = "\n\n# Project Instructions (ROLLO.md)\n" + "\n\n---\n\n".join(parts)
