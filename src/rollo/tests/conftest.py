...
用户的真实 ``~/.rollo`` 或旧版 ``~/.mini-claude`` 数据，也不会因为导入期常量
（``session.SESSION_DIR``）而绕过环境变量隔离。

隔离设计：
- 每个测试自动获得临时 HOME 与临时 ``ROLLO_RUNTIME_DIR``；
...
def isolated_runtime_env(tmp_path_factory: pytest.TempPathFactory, monkeypatch: pytest.MonkeyPatch):
    ...
    monkeypatch.setenv("ROLLO_RUNTIME_DIR", str(data_dir))
