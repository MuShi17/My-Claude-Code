...
def _patch_paths(user_dir: Path, project_dir: Path):
    ...
    home_patch = patch("rollo.skills.Path.home", return_value=user_dir)
    cwd_patch = patch("rollo.skills.Path.cwd", return_value=project_dir)
...
# ─── discovery: ~/.rollo/skills/ ──────────────────────────────


class TestDiscoverSkillsUserDir:
    """Verify discover_skills() picks up skills from ~/.rollo/skills/*/SKILL.md."""
    ...
    def test_single_user_skill(self, tmp_path: Path):
        ...
        _make_skill(
            user_home / ".rollo" / "skills", "user-skill",
            {"name": "user-skill", "description": "from home dir"},
        )

        reset_skill_cache()
        with patch("rollo.skills.Path.home", return_value=user_home), \
             patch("rollo.skills.Path.cwd", return_value=project_dir):
    ...
    def test_multiple_user_skills(self, tmp_path: Path):
        ...
        skills_dir = user_home / ".rollo" / "skills"
...
# ─── discovery: .rollo/skills/ (project) ─────────────────────
...
class TestDiscoverSkillsProjectDir:
    def test_project_skill_discovered(self, tmp_path: Path):
        ...
        _make_skill(
            project_dir / ".rollo" / "skills", "proj-skill",
...
class TestSkillOverride:
    def test_project_skill_overrides_user_skill_same_name(self, tmp_path: Path):
        ...
        _make_skill(
            user_home / ".rollo" / "skills", "shared-skill",
            {"name": "shared-skill", "description": "user version"},
        )
        _make_skill(
            project_dir / ".rollo" / "skills", "shared-skill",
...
class TestBuildSkillDescriptions:
    ...
    def test_auto_only_listed(self, tmp_path: Path):
        ...
        project_dir.mkdir()
        _make_skill(
            user_home / ".rollo" / "skills", "auto-run",
            {
                "name": "auto-run",
...
def test_reset_skill_cache_invalidates(tmp_path: Path):
    ...
    project_dir.mkdir()
    _make_skill(
        user_home / ".rollo" / "skills", "refresh",
