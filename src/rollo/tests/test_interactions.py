...
def test_subagents_inherit_ports_when_provided(
    ...
    monkeypatch.setattr("rollo.agent.Agent", _Capture)
    monkeypatch.setattr(agent, "_record_sub_agent_event", lambda **kwargs: None)
    monkeypatch.setattr(
        "rollo.agent.print_sub_agent_start", lambda *a, **k: None, raising=False
    )
    monkeypatch.setattr(
        "rollo.agent.print_sub_agent_end", lambda *a, **k: None, raising=False
    )
