...
def test_cli_fake_provider_covers_canonical_one_shot_list_latest_and_resume(
    ...
    session_root = home / ".rollo" / "sessions"
    ...
    monkeypatch.setattr(sys, "argv", ["rollo", *args, "one-shot"])
    cli_module.main()
    ...
    monkeypatch.setattr(sys, "argv", ["rollo", "--list"])
    cli_module.main()
    assert session_id in capsys.readouterr().out

    monkeypatch.setattr(sys, "argv", ["rollo", "--latest"])
    cli_module.main()
    assert f"Latest session: {session_id} (canonical)" in capsys.readouterr().out

    monkeypatch.setattr(sys, "argv", ["rollo", "--resume"])
    cli_module.main()
    ...
    monkeypatch.setattr(sys, "argv", ["rollo", *resume_args, "resume-turn"])
    cli_module.main()
...
def test_cli_legacy_files_are_ignored_and_old_authority_flags_are_rejected(
    ...
    (home / ".rollo" ).mkdir(parents=True, exist_ok=True)
    (home / ".rollo" / "runtime.sqlite").write_bytes(b"old root")
    ...
    monkeypatch.setattr(sys, "argv", ["rollo", "--log-authority", "shadow"])
