...
from mini_claude.project_context import (
...
def test_isolated_data_root_is_temporary(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    data_dir = Path(os.environ["MINI_CLAUDE_RUNTIME_DIR"])
...
def test_explicit_data_root_wins_over_environment(
    ...
    monkeypatch.setenv("MINI_CLAUDE_RUNTIME_DIR", str(tmp_path / "env-data"))
...
def test_project_skills_are_isolated_per_workspace(tmp_path: Path) -> None:
    ...
    from mini_claude.skills import discover_skills, reset_skill_cache
...
def test_memory_dirs_are_isolated_per_workspace(tmp_path: Path) -> None:
    ...
    from mini_claude.memory import get_memory_dir
...
def test_tool_dispatch_uses_context_not_process_cwd(
    ...
    from mini_claude.tools import execute_tool_value
...
def test_permission_rules_come_from_context_workspace(
    ...
    from mini_claude.tools import check_permission, load_permission_rules, reset_permission_cache
...
def test_new_file_confirmation_uses_context_root(
    ...
    from mini_claude.tools import check_permission, reset_permission_cache
...
def test_skill_fork_subagent_inherits_parent_context(
    ...
    ws.mkdir()
    process_cwd.mkdir()
    skill_dir = ws / ".claude" / "skills" / "forked"
...
def test_workspace_rules_are_loaded_from_context_root(
    ...
    """R7：`.claude/rules/*.md` 必须从 context 根加载，且与进程 cwd 无关。

    这条用例的存在是必需的：rules 加载曾被本 Change 的接线改坏（双重拼接
    `.claude/rules/.claude/rules`），而当时测试集没有任何 rules 覆盖，因此
    ...
    process_cwd.mkdir()
    (ws / ".claude" / "rules" / "team.md").write_text(
    ...
    assert "RULE-MARKER" in claude_md
    assert "RULE-MARKER" in prompt
    assert str(ws) in prompt or str(ws) in claude_md
...
def test_memory_index_is_written_into_context_memory_dir(tmp_path: Path) -> None:
    ...
    from mini_claude.memory import get_memory_dir
    from mini_claude.tools import _write_file
...
def test_cli_exits_with_diagnostic_when_workspace_unavailable(
    ...
    monkeypatch.setattr(Path, "cwd", staticmethod(_boom))
    monkeypatch.setattr(_sys, "argv", ["mini-claude", "hello"])
...
def test_agents_are_isolated_per_workspace(tmp_path: Path) -> None:
    ...
    from mini_claude.subagent import _discover_custom_agents, reset_agent_cache
...
def test_env_change_after_construction_is_declared_behaviour(
    ...
    monkeypatch.setenv("MINI_CLAUDE_RUNTIME_DIR", str(first_root))
    first = ProjectContext.from_root(workspace_root)

    monkeypatch.setenv("MINI_CLAUDE_RUNTIME_DIR", str(second_root))
