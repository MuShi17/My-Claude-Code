...
from mini_claude.artifact_archive import ArtifactArchive, ArtifactMetadataError
from mini_claude.runtime_lifecycle import DurableToolBoundary
from mini_claude.runtime_store import SQLiteRuntimeStore, StoreClosedError
...
def _patch_agent_runtime_paths(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr(
        "mini_claude.agent.runtime_data_dir",
        lambda: tmp_path,
    )
